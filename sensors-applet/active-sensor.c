...
#define SENSOR_VALUE_RANGE(x) \
  ((SensorValueRange)(CLAMP(x, VERY_LOW_SENSOR_VALUE, VERY_HIGH_SENSOR_VALUE)))

#define CAIRO_GRAPH_COLOR_GRADIENT 0.4

static const gchar *const temp_overlay_icons[] = {
    PIXMAPS_DIR "very-low-temp-icon.png", PIXMAPS_DIR "low-temp-icon.png",
    PIXMAPS_DIR "normal-temp-icon.png", PIXMAPS_DIR "high-temp-icon.png",
    PIXMAPS_DIR "very-high-temp-icon.png"};

static gdouble sensor_value_range_normalised(gdouble value, gdouble low_value,
                                             gdouble high_value) {
...
static SensorValueRange sensor_value_range(gdouble sensor_value,
                                           ...
                                           gdouble high_value) {
  gdouble range;
  range = sensor_value_range_normalised(sensor_value, low_value, high_value) *
          (gdouble)(VERY_HIGH_SENSOR_VALUE);
...
static gboolean active_sensor_execute_alarm(ActiveSensor *active_sensor,
  ...
  g_debug("EXECUTING %s ALARM: %s", (notif_type == LOW_ALARM ? "LOW" : "HIGH"),
          active_sensor->alarm_command[notif_type]);
  ret = g_spawn_command_line_async(active_sensor->alarm_command[notif_type],
                                   &error);
...
/* needs to be able to be called by the config dialog when the alarm command
 * changes */
...
static void active_sensor_alarm_on(ActiveSensor *active_sensor,
  ...
  if (gtk_tree_model_get_iter(model, &iter, tree_path)) {
    if (active_sensor->alarm_timeout_id[notif_type] == -1) {
      ...
      g_debug("Activating alarm to repeat every %d seconds",
      ...
      int timeout =
          (active_sensor->alarm_timeout <= 0 ? G_MAXINT
                                             : active_sensor->alarm_timeout);
      switch (notif_type) {
        case LOW_ALARM:
          active_sensor->alarm_timeout_id[notif_type] = g_timeout_add_seconds(
              timeout, (GSourceFunc)active_sensor_execute_low_alarm,
              active_sensor);
          break;

        case HIGH_ALARM:
          active_sensor->alarm_timeout_id[notif_type] = g_timeout_add_seconds(
              timeout, (GSourceFunc)active_sensor_execute_high_alarm,
              active_sensor);
          break;
...
static void active_sensor_update_icon(ActiveSensor *active_sensor,
  ...
  if (overlay_icon_filename) {
    ...
    overlay_icon = gdk_pixbuf_new_from_file_at_size(
        overlay_icon_filename, DEFAULT_ICON_SIZE, DEFAULT_ICON_SIZE, NULL);

    if (overlay_icon) {
      gdk_pixbuf_composite(overlay_icon, new_icon, 0, 0, DEFAULT_ICON_SIZE,
                           DEFAULT_ICON_SIZE, 0, 0, 1.0, 1.0,
  ...
  gtk_image_set_from_pixbuf(GTK_IMAGE(active_sensor->icon), new_icon);
...
static void active_sensor_update_graph(ActiveSensor *as, cairo_t *cr) {
  ...
  /* so we can set a clipping area, as well as fill the * back of the graph
   * black */
  ...
  for (i = 0; i < MIN(as->num_samples, width); i++) {
    ...
    line_height =
        sensor_value_range_normalised(
            as->sensor_values[i], as->sensor_low_value, as->sensor_high_value) *
        height;
  ...
  cairo_pattern_add_color_stop_rgb(
      pattern, 0, as->graph_color.red - CAIRO_GRAPH_COLOR_GRADIENT,
      as->graph_color.green - CAIRO_GRAPH_COLOR_GRADIENT,
      as->graph_color.blue - CAIRO_GRAPH_COLOR_GRADIENT);

  cairo_pattern_add_color_stop_rgb(
      pattern, height, as->graph_color.red + CAIRO_GRAPH_COLOR_GRADIENT,
      as->graph_color.green + CAIRO_GRAPH_COLOR_GRADIENT,
      as->graph_color.blue + CAIRO_GRAPH_COLOR_GRADIENT);
...
gboolean graph_draw_cb(GtkWidget *graph, cairo_t *cr, gpointer data) {
  ActiveSensor *as;
...
static void active_sensor_set_graph_dimensions(ActiveSensor *as, gint width,
                                               gint height) {
  ...
  graph_width = CLAMP(width - GRAPH_FRAME_EXTRA_WIDTH, 1,
                      width - GRAPH_FRAME_EXTRA_WIDTH);
  graph_height = CLAMP(height - GRAPH_FRAME_EXTRA_WIDTH, 1,
                       height - GRAPH_FRAME_EXTRA_WIDTH);
  ...
  if (as->sensor_values) {
    ...
    memcpy(as->sensor_values, old_values,
  ...
  gtk_widget_set_size_request(as->graph, graph_width, graph_height);
}

void active_sensor_update_graph_dimensions(ActiveSensor *as, gint sizes[2]) {
...
ActiveSensor *active_sensor_new(SensorsApplet *sensors_applet,
  ...
  gtk_container_add(GTK_CONTAINER(active_sensor->graph_frame),
                    active_sensor->graph);
  gtk_widget_add_events(active_sensor->graph_frame, GDK_ALL_EVENTS_MASK);

  /* need to set size according to orientation */
  orient = mate_panel_applet_get_orient(active_sensor->sensors_applet->applet);
  graph_size =
      g_settings_get_int(active_sensor->sensors_applet->settings, GRAPH_SIZE);

  horizontal = ((orient == MATE_PANEL_APPLET_ORIENT_UP) ||
                (orient == MATE_PANEL_APPLET_ORIENT_DOWN));

  active_sensor_set_graph_dimensions(
      active_sensor, (horizontal ? graph_size : sensors_applet->size),
      (horizontal ? sensors_applet->size : graph_size));

  g_signal_connect(G_OBJECT(active_sensor->graph), "draw",
                   G_CALLBACK(graph_draw_cb), active_sensor);
...
static void active_sensor_update_sensor_value(ActiveSensor *as,
  ...
  if (as->num_samples > 1) {
    memmove(&(as->sensor_values[1]), as->sensor_values,
...
void active_sensor_update(ActiveSensor *active_sensor,
  ...
  if (gtk_tree_model_get_iter(model, &iter, path)) {
    ...
    gtk_tree_model_get(
        GTK_TREE_MODEL(sensors_applet->sensors), &iter, PATH_COLUMN,
        &sensor_path, ID_COLUMN, &sensor_id, LABEL_COLUMN, &sensor_label,
        INTERFACE_COLUMN, &sensor_interface, SENSOR_TYPE_COLUMN, &sensor_type,
        ENABLE_COLUMN, &sensor_enabled, LOW_VALUE_COLUMN, &sensor_low_value,
        HIGH_VALUE_COLUMN, &sensor_high_value, ALARM_ENABLE_COLUMN,
        &sensor_alarm_enabled, MULTIPLIER_COLUMN, &sensor_multiplier,
        OFFSET_COLUMN, &sensor_offset, ICON_PIXBUF_COLUMN, &icon_pixbuf,
    ...
    if ((get_sensor_value = sensors_applet_plugins_get_sensor_value_func(
             sensors_applet, sensor_interface)) != NULL) {
      sensor_value =
          get_sensor_value(sensor_path, sensor_id, sensor_type, &error);
      ...
      if (error) {
        ...
        switch (sensor_type) {
          case TEMP_SENSOR:
            scale = (TemperatureScale)g_settings_get_int(
                sensors_applet->settings, TEMPERATURE_SCALE);
            /* scale value */
            ...
            switch (scale) {
              case FAHRENHEIT:
                value_text =
                    g_strdup_printf("%2.0f %s", sensor_value,
                                    (hide_units ? "" : UNITS_FAHRENHEIT));
                /* tooltip should
                 * always display
                 * units */
                value_tooltip =
                    g_strdup_printf("%2.0f %s", sensor_value, UNITS_FAHRENHEIT);

                break;
              case CELSIUS:
                value_text = g_strdup_printf("%2.0f %s", sensor_value,
                                             (hide_units ? "" : UNITS_CELSIUS));
                value_tooltip =
                    g_strdup_printf("%2.0f %s", sensor_value, UNITS_CELSIUS);
                break;
              case KELVIN:
          ...
          case FAN_SENSOR:
            sensor_value = (sensor_value * sensor_multiplier) + sensor_offset;
            value_text = g_strdup_printf("%4.0f %s", sensor_value,
                                         (hide_units ? "" : UNITS_RPM));
            value_tooltip =
                g_strdup_printf("%4.0f %s", sensor_value, UNITS_RPM);
            break;

          case VOLTAGE_SENSOR:
            sensor_value = (sensor_value * sensor_multiplier) + sensor_offset;
            value_text = g_strdup_printf("%4.2f %s", sensor_value,
                                         (hide_units ? "" : UNITS_VOLTAGE));
            value_tooltip =
                g_strdup_printf("%4.2f %s", sensor_value, UNITS_VOLTAGE);
            break;

          case CURRENT_SENSOR:
            sensor_value = (sensor_value * sensor_multiplier) + sensor_offset;
            value_text = g_strdup_printf("%4.2f %s", sensor_value,
                                         (hide_units ? "" : UNITS_CURRENT));
            value_tooltip =
                g_strdup_printf("%4.2f %s", sensor_value, UNITS_CURRENT);
            break;
      ...
      if (display_mode == DISPLAY_ICON ||
        ...
        if ((sensor_value_range(sensor_value, sensor_low_value,
                                sensor_high_value) !=
      ...
      if (sensor_alarm_enabled) {
        if (sensor_value >= sensor_high_value ||
          ...
          if (display_mode == DISPLAY_LABEL_WITH_VALUE ||
            ...
            value_text = g_markup_printf_escaped(
                "<span foreground=\"#FF0000\">%s</span>", old_value_text);
      ...
      if (display_mode == DISPLAY_LABEL_WITH_VALUE ||
        ...
        if (font_size) {
          ...
          value_text = g_strdup_printf("<span font_desc=\"%d\">%s</span>",
                                       font_size, old_value_text);
          g_free(old_value_text);
        }

        /*Keep the label as large as previous size unless hide_units has changed
         */
        gtk_widget_get_preferred_size(GTK_WIDGET(active_sensor->value), &req,
                                      NULL);
      ...
      if (display_mode == DISPLAY_LABEL_WITH_VALUE) {
        if (font_size) {
          ...
          sensor_label = g_strdup_printf("<span font_desc=\"%d\">%s</span>",
                                         font_size, old_value_text);
          g_free(old_value_text);
        }
      ...
      g_debug("no get_sensor_value function yet installed for interface %s.",
              sensor_interface);
    }
...
void active_sensor_icon_changed(ActiveSensor *active_sensor,
  ...
  if (gtk_tree_model_get_iter(model, &iter, path)) {
    gtk_tree_model_get(GTK_TREE_MODEL(sensors_applet->sensors), &iter,
                       SENSOR_TYPE_COLUMN, &sensor_type, ICON_PIXBUF_COLUMN,
                       &icon_pixbuf, -1);
