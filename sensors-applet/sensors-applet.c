...
static void destroy_cb(GtkWidget *widget, gpointer data) {
  ...
  /* destroy dialogs, remove timeout and clear sensors tree and finally the
   * applet */
  ...
  g_list_foreach(sensors_applet->active_sensors, (GFunc)active_sensor_destroy,
                 NULL);
...
static void change_orient_cb(MatePanelApplet *applet,
                             MatePanelAppletOrient orient, gpointer data) {
  SensorsApplet *sensors_applet;
  sensors_applet = (SensorsApplet *)data;
...
static void size_allocate_cb(MatePanelApplet *applet, GtkAllocation *allocation,
                             gpointer data) {
  SensorsApplet *sensors_applet;
  ...
  if ((orient == MATE_PANEL_APPLET_ORIENT_LEFT) ||
    ...
    if (sensors_applet->size == allocation->width) return;
    sensors_applet->size = allocation->width;
  } else {
    if (sensors_applet->size == allocation->height) return;
...
static void style_set_cb(GtkWidget *widget, GtkStyle *old_style,
                         gpointer data) {
  ...
  if (sensors_applet->sensors) {
    for (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(sensors_applet->sensors),
                                       &interfaces_iter);
         not_end_of_interfaces;
         not_end_of_interfaces = gtk_tree_model_iter_next(
             GTK_TREE_MODEL(sensors_applet->sensors), &interfaces_iter)) {
      /* reset sensors sentinel */
      not_end_of_sensors = TRUE;

      for (gtk_tree_model_iter_children(GTK_TREE_MODEL(sensors_applet->sensors),
                                        &sensors_iter, &interfaces_iter);
           not_end_of_sensors;
           not_end_of_sensors = gtk_tree_model_iter_next(
               GTK_TREE_MODEL(sensors_applet->sensors), &sensors_iter)) {
        GdkPixbuf *new_icon;

        gtk_tree_model_get(GTK_TREE_MODEL(sensors_applet->sensors),
                           &sensors_iter, ENABLE_COLUMN, &enabled,
                           ICON_TYPE_COLUMN, &icon_type, -1);
        /* update icons */
        new_icon = sensors_applet_load_icon(icon_type);

        gtk_tree_store_set(sensors_applet->sensors, &sensors_iter,
                           ICON_PIXBUF_COLUMN, new_icon, -1);
        g_object_unref(new_icon);

        /* update icons only if currently being displayed */
        if (enabled && (display_mode == DISPLAY_ICON ||
...
static gboolean mouse_enter_cb(GtkWidget *widget, GdkEventCrossing *event,
                               gpointer data) {
...
static gboolean mouse_leave_cb(GtkWidget *widget, GdkEventCrossing *event,
                               gpointer data) {
  SensorsApplet *sensor_applet = data;
...
static const GtkActionEntry sensors_applet_menu_actions[] = {
    {"Preferences", "document-properties", N_("_Preferences"), NULL, NULL,
     G_CALLBACK(prefs_cb)},
    {"Help", "help-browser", N_("_Help"), NULL, NULL, G_CALLBACK(help_cb)},
    {"About", "help-about", N_("_About"), NULL, NULL, G_CALLBACK(about_cb)}};

void sensors_applet_notify_active_sensor(ActiveSensor *active_sensor,
  ...
  if (gtk_tree_model_get_iter(GTK_TREE_MODEL(sensors_applet->sensors), &iter,
                              path)) {
    gtk_tree_model_get(GTK_TREE_MODEL(sensors_applet->sensors), &iter,
                       LABEL_COLUMN, &sensor_label, PATH_COLUMN, &sensor_path,
  ...
  switch (notif_type) {
    ...
    case HIGH_ALARM:
      ...
      } else if (active_sensor->sensor_values[0] >=
      ...
      switch ((SensorType)sensor_type) {
        case TEMP_SENSOR:
          ...
          temp_scale = (TemperatureScale)g_settings_get_int(
              active_sensor->sensors_applet->settings, TEMPERATURE_SCALE);
      ...
      timeout_msecs = (active_sensor->alarm_timeout
                           ? MIN(DEFAULT_NOTIFY_TIMEOUT,
                                 (active_sensor->alarm_timeout * 1000))
                           : DEFAULT_NOTIFY_TIMEOUT);

      summary = g_strdup_printf("%s %s %s", sensor_label, unit_type_title,
                                _("Alarm"));
      message =
          g_strdup_printf("%s %s %s (%s %2.0f%s)", sensor_label, unit_type,
    ...
    case SENSOR_INTERFACE_ERROR:
      ...
      /* if the last error happened less than 10 seconds ago, don't display this
       * one this should prevent recurring popups for removed sensors, like
       * USB-HDDs */
      if (seconds < 11.0) {
      ...
      message = g_strdup_printf(_("An error occurred while trying to update "
                                  "the value of the sensor %s located at %s."),
                                sensor_label, sensor_path);
      timeout_msecs =
          g_settings_get_int(active_sensor->sensors_applet->settings, TIMEOUT);
  ...
  if (show_notification) {
    active_sensor_libnotify_notify(active_sensor, notif_type, summary, message,
                                   "dialog-warning", timeout_msecs);
  }
...
void sensors_applet_notify_end_all(SensorsApplet *sensors_applet) {
  ...
  g_list_foreach(sensors_applet->active_sensors,
                 (GFunc)sensors_applet_notify_end_all_gfunc, NULL);
#endif
}
...
static void sensors_applet_pack_display_empty_grid_cb(GtkWidget *widget,
                                                      gpointer data) {
...
static void sensors_applet_pack_display(SensorsApplet *sensors_applet) {
  ...
  if (sensors_applet->active_sensors == NULL ||
    ...
    no_sensors_enabled_label =
        g_object_new(GTK_TYPE_LABEL, "label", _("No sensors enabled!"), NULL);

    if (sensors_applet->grid == NULL) {
      /* only need 1 row and 1 col */
      sensors_applet->grid = gtk_grid_new();
      gtk_grid_set_column_spacing(GTK_GRID(sensors_applet->grid),
                                  COLUMN_SPACING);
      ...
      /* add grid to applet */
      gtk_container_add(GTK_CONTAINER(sensors_applet->applet),
                        sensors_applet->grid);
    } else {
    ...
    gtk_grid_attach(GTK_GRID(sensors_applet->grid),
                    GTK_WIDGET(no_sensors_enabled_label), 0, 0, 1, 1);
  ...
  display_mode =
      (DisplayMode)g_settings_get_int(sensors_applet->settings, DISPLAY_MODE);
  layout_mode =
      (LayoutMode)g_settings_get_int(sensors_applet->settings, LAYOUT_MODE);

  horizontal = (((mate_panel_applet_get_orient(sensors_applet->applet) ==
  ...
  switch (display_mode) {
    case DISPLAY_VALUE:
      gtk_widget_get_preferred_size(GTK_WIDGET(first_sensor->value), &req,
                                    NULL);
      ...
      num_sensors_per_group =
          (sensors_applet->size / (horizontal ? value_height : value_width));
      break;
    ...
    case DISPLAY_LABEL_WITH_VALUE:
      ...
      gtk_widget_get_preferred_size(GTK_WIDGET(first_sensor->label), &req,
                                    NULL);
      ...
      switch (layout_mode) {
        ...
        case VALUE_BELOW_LABEL:
          num_sensors_per_group =
              (sensors_applet->size / (horizontal
    ...
    case DISPLAY_ICON:
      ...
      num_sensors_per_group =
          (sensors_applet->size / (horizontal ? icon_height : icon_width));
      break;
  ...
  if (sensors_applet->grid == NULL) {
    ...
    old_grid_children =
        gtk_container_get_children(GTK_CONTAINER(sensors_applet->grid));
  ...
  /* if showing labels / icons and values, need to pack labels / icons these
   * first */
  if (display_mode == DISPLAY_ICON_WITH_VALUE ||
      display_mode == DISPLAY_LABEL_WITH_VALUE) {
    /* loop through columns */
    for (i = 0; current_sensor != NULL && i < cols;
         /* increments depends on how we lay them out - see below */) {
      ...
      for (j = 0; current_sensor && j < rows; /* see bottom of for loop*/) {
        ...
        if (display_mode == DISPLAY_ICON_WITH_VALUE) {
          if (((ActiveSensor *)(current_sensor->data))->icon) {
            gtk_grid_attach(GTK_GRID(sensors_applet->grid),
                            ((ActiveSensor *)(current_sensor->data))->icon, i,
                            j, 1, 1);
          }
        } else {
          if (((ActiveSensor *)(current_sensor->data))->label) {
            gtk_grid_attach(GTK_GRID(sensors_applet->grid),
                            ((ActiveSensor *)(current_sensor->data))->label, i,
                            j, 1, 1);
          }
        }
        ...
        if (layout_mode == VALUE_BESIDE_LABEL) {
          ...
          if (((ActiveSensor *)(current_sensor->data))->label) {
            gtk_label_set_xalign(
                GTK_LABEL(((ActiveSensor *)(current_sensor->data))->label), 0);
            gtk_label_set_yalign(
                GTK_LABEL(((ActiveSensor *)(current_sensor->data))->label),
                0.5);
          }
          ...
          if (((ActiveSensor *)(current_sensor->data))->value) {
            gtk_grid_attach(GTK_GRID(sensors_applet->grid),
                            ((ActiveSensor *)(current_sensor->data))->value, i,
                            j + 1, 1, 1);
          }
          j += 2;
        }
  ...
  } else { /* not showing labels and icons with values, so just pack either only
              icons or values */
      ...
      for (j = 0; current_sensor != NULL && j < rows; ++j) {
        if (display_mode == DISPLAY_VALUE) {
          ...
          if (((ActiveSensor *)(current_sensor->data))->graph) {
            gtk_grid_attach(
                GTK_GRID(sensors_applet->grid),
                ((ActiveSensor *)(current_sensor->data))->graph_frame, i, j, 1,
                1);
          }
        }
...
GdkPixbuf *sensors_applet_load_icon(IconType icon_type) {
  ...
  icon = gtk_icon_theme_load_icon(icon_theme, stock_icons[icon_type],
                                  DEFAULT_ICON_SIZE,
  ...
  if (error) {
    ...
    icon =
        gtk_icon_theme_load_icon(icon_theme, "image-missing", DEFAULT_ICON_SIZE,
                                 GTK_ICON_LOOKUP_USE_BUILTIN, &error);

    if (error) {
      /* this will quit sensors-applet but it is a pretty major error so may as
       * well */
      g_error("Could not load GTK_STOCK_MISSING_IMAGE - major error!!!: %s",
              error->message);
      g_error_free(error);
      error = NULL;
    }
...
gboolean sensors_applet_add_sensor(
    SensorsApplet *sensors_applet, const gchar *path, const gchar *id,
    const gchar *label, const gchar *interface, SensorType type,
    gboolean enable, gdouble low_value, gdouble high_value,
    gboolean alarm_enable, const gchar *low_alarm_command,
    const gchar *high_alarm_command, gint alarm_timeout, gdouble multiplier,
    gdouble offset, IconType icon_type, const gchar *graph_color) {
  GtkTreeIter interfaces_iter, sensors_iter;
  ...
  if (NULL == sensors_applet->sensors) {
    sensors_applet->sensors =
        gtk_tree_store_new(N_COLUMNS, G_TYPE_STRING, /* path */
                           G_TYPE_STRING,            /* id */
                           G_TYPE_STRING,            /* label */
  ...
  for (not_empty_tree = gtk_tree_model_get_iter_first(
           GTK_TREE_MODEL(sensors_applet->sensors), &interfaces_iter);
       not_empty_tree && not_end_of_interfaces && !interface_exists;
       not_end_of_interfaces = gtk_tree_model_iter_next(
           GTK_TREE_MODEL(sensors_applet->sensors), &interfaces_iter)) {
    gtk_tree_model_get(GTK_TREE_MODEL(sensors_applet->sensors),
                       &interfaces_iter, INTERFACE_COLUMN, &node_interface, -1);
    if (g_ascii_strcasecmp(interface, node_interface) == 0) {
      /* found interface in tree */
      interface_exists = TRUE;

      /* now see if this actual sensor already exists within this interface -
       * don't want to add duplicates */
      /* see if have children */
      for (not_end_of_sensors = gtk_tree_model_iter_children(
               GTK_TREE_MODEL(sensors_applet->sensors), &sensors_iter,
               &interfaces_iter);
           not_end_of_sensors;
           not_end_of_sensors = gtk_tree_model_iter_next(
               GTK_TREE_MODEL(sensors_applet->sensors), &sensors_iter)) {
        gtk_tree_model_get(GTK_TREE_MODEL(sensors_applet->sensors),
                           &sensors_iter, PATH_COLUMN, &sensor_path, ID_COLUMN,
                           &sensor_id, SENSOR_TYPE_COLUMN, &sensor_type, -1);
        if (g_ascii_strcasecmp(sensor_id, id) == 0 &&
            g_ascii_strcasecmp(sensor_path, path) == 0 && sensor_type == type) {
          /* sensor already exists so dont add a second time */
          g_debug(
              "sensor with path: %s, id: %s already exists in tree, not adding "
              "a second time",
              sensor_path, sensor_id);
          g_free(sensor_id);
          g_free(sensor_path);
  ...
  if (!interface_exists) {
    ...
    g_hash_table_insert(sensors_applet->required_plugins, g_strdup(interface),
                        GINT_TO_POINTER(TRUE));
    g_debug("added interface %s to required plugins", interface);

    /* wasn't able to find interface root node so create it */
    gtk_tree_store_append(sensors_applet->sensors, &interfaces_iter, NULL);

    gtk_tree_store_set(sensors_applet->sensors, &interfaces_iter, ID_COLUMN,
                       interface, INTERFACE_COLUMN, interface, VISIBLE_COLUMN,
                       FALSE, -1);
  ...
  gtk_tree_store_append(sensors_applet->sensors, &sensors_iter,
                        &interfaces_iter);

  /* if sensor is already in settings, load values from there */
  gchar *applet_path =
      mate_panel_applet_get_preferences_path(sensors_applet->applet);
  ...
  GSettings *settings =
      g_settings_new_with_path("org.mate.sensors-applet.sensor", settings_path);
  ...
  if (settings_id != NULL && settings_id[0] != '\0') {
    ...
    gtk_tree_store_set(
        sensors_applet->sensors, &sensors_iter, PATH_COLUMN,
        g_settings_get_string(settings, PATH), ID_COLUMN, settings_id,
        LABEL_COLUMN, g_settings_get_string(settings, LABEL), INTERFACE_COLUMN,
        g_settings_get_string(settings, INTERFACE), SENSOR_TYPE_COLUMN,
        g_settings_get_int(settings, SENSOR_TYPE), ENABLE_COLUMN, enable,
        VISIBLE_COLUMN, TRUE, LOW_VALUE_COLUMN,
        g_settings_get_double(settings, LOW_VALUE), HIGH_VALUE_COLUMN,
        g_settings_get_double(settings, HIGH_VALUE), ALARM_ENABLE_COLUMN,
        g_settings_get_boolean(settings, ALARM_ENABLED), ALARM_TIMEOUT_COLUMN,
        g_settings_get_int(settings, ALARM_TIMEOUT), LOW_ALARM_COMMAND_COLUMN,
        g_settings_get_string(settings, LOW_ALARM_COMMAND),
        HIGH_ALARM_COMMAND_COLUMN,
        g_settings_get_string(settings, HIGH_ALARM_COMMAND), MULTIPLIER_COLUMN,
        g_settings_get_double(settings, MULTIPLIER), OFFSET_COLUMN,
        g_settings_get_double(settings, OFFSET), ICON_TYPE_COLUMN,
        g_settings_get_int(settings, ICON_TYPE), ICON_PIXBUF_COLUMN, icon,
        GRAPH_COLOR_COLUMN, g_settings_get_string(settings, GRAPH_COLOR), -1);
    g_free(settings_id);
  } else {
    icon = sensors_applet_load_icon(icon_type);
    gtk_tree_store_set(
        sensors_applet->sensors, &sensors_iter, PATH_COLUMN, path, ID_COLUMN,
        id, LABEL_COLUMN, label, INTERFACE_COLUMN, interface,
        SENSOR_TYPE_COLUMN, type, ENABLE_COLUMN, enable, VISIBLE_COLUMN, TRUE,
        LOW_VALUE_COLUMN, low_value, HIGH_VALUE_COLUMN, high_value,
        ALARM_ENABLE_COLUMN, alarm_enable, ALARM_TIMEOUT_COLUMN, alarm_timeout,
        LOW_ALARM_COMMAND_COLUMN, low_alarm_command, HIGH_ALARM_COMMAND_COLUMN,
        high_alarm_command, MULTIPLIER_COLUMN, multiplier, OFFSET_COLUMN,
        offset, ICON_TYPE_COLUMN, icon_type, ICON_PIXBUF_COLUMN, icon,
        GRAPH_COLOR_COLUMN, graph_color, -1);
  }
...
void sensors_applet_alarm_off(SensorsApplet *sensors_applet, GtkTreePath *path,
  ...
  if ((active_sensor =
           sensors_applet_find_active_sensor(sensors_applet, path)) != NULL) {
...
void sensors_applet_sensor_enabled(SensorsApplet *sensors_applet,
  ...
  active_sensor = active_sensor_new(
      sensors_applet, gtk_tree_row_reference_new(
                          GTK_TREE_MODEL(sensors_applet->sensors), path));
  active_sensor_update(active_sensor, sensors_applet);

  /* keep list sorted */
  sensors_applet->active_sensors =
      g_list_insert_sorted(sensors_applet->active_sensors, active_sensor,
...
void sensors_applet_reorder_sensors(SensorsApplet *sensors_applet) {
  sensors_applet->active_sensors = g_list_sort(
      sensors_applet->active_sensors, (GCompareFunc)active_sensor_compare);
...
void sensors_applet_sensor_disabled(SensorsApplet *sensors_applet,
  ...
  if ((active_sensor =
    ...
    sensors_applet->active_sensors =
        g_list_remove(sensors_applet->active_sensors, active_sensor);
...
gboolean sensors_applet_update_active_sensors(SensorsApplet *sensors_applet) {
  ...
  if (sensors_applet->active_sensors) {
    g_list_foreach(sensors_applet->active_sensors, (GFunc)active_sensor_update,
                   sensors_applet);
    return TRUE;
  }
  return FALSE;
...
void sensors_applet_graph_size_changed(SensorsApplet *sensors_applet) {
  ...
  if (sensors_applet->active_sensors) {
    ...
    g_list_foreach(sensors_applet->active_sensors,
                   (GFunc)active_sensor_update_graph_dimensions, &dimensions);
  }
}

gdouble sensors_applet_convert_temperature(gdouble value, TemperatureScale old,
...
void sensors_applet_init(SensorsApplet *sensors_applet) {
  ...
  mate_panel_applet_set_background_widget(
      MATE_PANEL_APPLET(sensors_applet->applet),
  ...
  sensors_applet->required_plugins =
  ...
  g_signal_connect(sensors_applet->applet, "destroy", G_CALLBACK(destroy_cb),
                   sensors_applet);

  /* init gsettings */
  sensors_applet->settings = mate_panel_applet_settings_new(
      sensors_applet->applet, "org.mate.sensors-applet");
  ...
  /* should have created sensors tree above, but if have not was because we
   * couldn't find any sensors */
  if (NULL == sensors_applet->sensors) {
    GtkWidget *label;
  ...
  gtk_action_group_add_actions(action_group, sensors_applet_menu_actions,
                               G_N_ELEMENTS(sensors_applet_menu_actions),
                               sensors_applet);
  ui_path = g_build_filename(UIDIR, SENSORS_APPLET_MENU_FILE, NULL);
  mate_panel_applet_setup_menu_from_file(sensors_applet->applet, ui_path,
                                         action_group);
  g_free(ui_path);
  ...
  g_signal_connect(G_OBJECT(sensors_applet->applet), "leave_notify_event",
                   G_CALLBACK(mouse_leave_cb), (gpointer)sensors_applet);

  g_signal_connect(G_OBJECT(sensors_applet->applet), "enter_notify_event",
                   G_CALLBACK(mouse_enter_cb), (gpointer)sensors_applet);
