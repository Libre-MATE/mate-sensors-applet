...
/* these must be as the gsettings schema in file
 * org.mate.sensors-applet.gschema.xml.in gsettings gvariant type string */
#define GSGVTS "s"
/* gsettings gvariant type string for the array */
#define GSGVTSA "as"

gchar *sensors_applet_settings_get_unique_id(const gchar *interface,
                                             const gchar *id,
                                             const gchar *path) {
...
gboolean sensors_applet_settings_load_sensors(SensorsApplet *sensors_applet) {
  ...
  while (g_variant_iter_loop(iter, GSGVTS, &gsuid)) {
     ...
     * sensors_applet_add_sensor() gtk_tree_store_set() copies strings, so free
     * them */
    sensors_applet_add_sensor(
        sensors_applet, current_path = g_settings_get_string(settings, PATH),
        ...
        current_high_alarm_command =
            g_settings_get_string(settings, HIGH_ALARM_COMMAND),
...
static void sensors_applet_settings_print_sensors_tree(
  ...
  for (not_end_of_interfaces = gtk_tree_model_get_iter_first(
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(sensors_applet->sensors),
                       &interfaces_iter, INTERFACE_COLUMN, &interface_name, -1);
    ...
    for (not_end_of_sensors = gtk_tree_model_iter_children(
      ...
      gtk_tree_model_get(GTK_TREE_MODEL(sensors_applet->sensors), &sensors_iter,
                         PATH_COLUMN, &sensor_path, ID_COLUMN, &sensor_id, -1);

      sensor_hash = sensors_applet_settings_get_unique_id(
          interface_name, sensor_id, sensor_path);
...
static gint sensors_applet_settings_sort_sensors_iter_compare(
    SensorsApplet *sensors_applet, GtkTreeIter ti_a, GtkTreeIter ti_b) {
  GtkTreePath *tp_a;
  GtkTreePath *tp_b;
  gint ret_val;

  tp_a =
      gtk_tree_model_get_path(GTK_TREE_MODEL(sensors_applet->sensors), &ti_a);
  tp_b =
      gtk_tree_model_get_path(GTK_TREE_MODEL(sensors_applet->sensors), &ti_b);
...
static gint sensors_applet_settings_sort_sensors_sort(
    SensorsApplet *sensors_applet, const gchar *hash_a, const gchar *hash_b,
    gboolean a_is_first) {
  g_assert(sensors_applet);
  g_assert(sensors_applet->sensors);
  ...
  if (found_a && found_b) {
    ...
    if (a_is_first) {
      /* this 'fails' if the tree is empty, but this is already checked with the
       * first loop */
      gtk_tree_model_get_iter_first(GTK_TREE_MODEL(sensors_applet->sensors),
                                    &first_iter);

      /* we are only interested in the case, where the two are not equal
       * that means the a's interface is not the first interface */
      if (0 != sensors_applet_settings_sort_sensors_iter_compare(
                   sensors_applet, first_iter, interface_iter_a)) {
        /* it should be, so it needs to be moved to the first position */
        gtk_tree_store_move_after(GTK_TREE_STORE(sensors_applet->sensors),
                                  &interface_iter_a, NULL);
      }
    }

    /* check b's interface position
     * (1) if it is before a's if. - not possible, bc I have just set a's if. as
     * first afterwards every new if. will be set right after (one) a's if. (0)
     * if it is the same as a's, noop
     * (-1) if it is after a, check if it is right after a, if not move it there
     */
    ret_val = sensors_applet_settings_sort_sensors_iter_compare(
        sensors_applet, interface_iter_a, interface_iter_b);
    if (-1 == ret_val) {
      /* set iter_next to the iter after a's if. iter, can copy like this */
      iter_next = interface_iter_a;
      /* this 'fails' if there is no node after a's if. iter, but we already
       * know, that at least b's if. iter is */
      gtk_tree_model_iter_next(GTK_TREE_MODEL(sensors_applet->sensors),
                               &iter_next);

      /* the node right after a's if. iter is not b's if. iter, so move b's if.
       * iter */
      if (0 != sensors_applet_settings_sort_sensors_iter_compare(
                   sensors_applet, iter_next, interface_iter_b)) {
        gtk_tree_store_move_after(GTK_TREE_STORE(sensors_applet->sensors),
                                  &interface_iter_b, &interface_iter_a);
      }
    }

    /* at this point the interfaces are sorted, the sensors are next */
    /* set a to be the first sensor in the sensors tree, under the first if.
     * iter */
    if (a_is_first) {
      /* this 'fails' if the tree is empty, but at least a is in it */
      gtk_tree_model_iter_children(GTK_TREE_MODEL(sensors_applet->sensors),
                                   &first_iter, &interface_iter_a);

      /* we are only interested in the case, where the two are not equal
       * that means the a is not the first sensor */
      if (0 != sensors_applet_settings_sort_sensors_iter_compare(
                   sensors_applet, first_iter, sensor_iter_a)) {
        /* it should be, so it needs to be moved to the first position */
        gtk_tree_store_move_after(GTK_TREE_STORE(sensors_applet->sensors),
                                  &sensor_iter_a, NULL);
      }
     ...
     */
    if (0 == ret_val &&
        -1 == sensors_applet_settings_sort_sensors_iter_compare(
                  sensors_applet, sensor_iter_a, sensor_iter_b)) {
      /* set iter_next to the iter after a's iter, can copy like this */
      iter_next = sensor_iter_a;
      /* this 'fails' if there is no node after a's iter, but we already know,
       * that at least b's iter is */
      gtk_tree_model_iter_next(GTK_TREE_MODEL(sensors_applet->sensors),
                               &iter_next);

      /* the node right after a's iter is not b's iter, so move b's iter */
      if (0 != sensors_applet_settings_sort_sensors_iter_compare(
                   sensors_applet, iter_next, sensor_iter_b)) {
        gtk_tree_store_move_after(GTK_TREE_STORE(sensors_applet->sensors),
                                  &sensor_iter_b, &sensor_iter_a);
      }
    }
    ...
    if (-1 == ret_val) {
      ...
      gtk_tree_model_iter_children(GTK_TREE_MODEL(sensors_applet->sensors),
                                   &first_iter, &interface_iter_b);

      /* we are only interested in the case, where the two are not equal
       * that means the b is not the first sensor */
      if (0 != sensors_applet_settings_sort_sensors_iter_compare(
                   sensors_applet, first_iter, sensor_iter_b)) {
        /* it should be, so it needs to be moved to the first position */
        gtk_tree_store_move_after(GTK_TREE_STORE(sensors_applet->sensors),
                                  &sensor_iter_b, NULL);
      }
...
gboolean sensors_applet_settings_sort_sensors(SensorsApplet *sensors_applet) {
   ...
   * for an empty array a pointer to a NULL pointer will be returned
   * (g_variant_dup_strv ()) */
  ...
  for (i = 0; (NULL != sensors_applet->sensors_hash_array[i]) &&
              (NULL != sensors_list[i]);
       i++) {
    if (g_ascii_strcasecmp(sensors_applet->sensors_hash_array[i],
                           sensors_list[i]) != 0) {
      hle = FALSE;
      break;
    }
...
#ifdef SORT_DEBUG
    syslog(LOG_ERR,
           "sensor sort: saved list is the same as the new one, returning");
#endif
  ...
  for (i = 0; NULL != sensors_list[i]; i++) {
    ...
    /* now that we have two hashes, find the two corresponding sensors and sort
     * them if i == 1, we have both a and b set, this is the first time calling
     * this function */
    ret_val = sensors_applet_settings_sort_sensors_sort(
        sensors_applet, hash_a, hash_b, (i == first_counter));
    ...
    /* after sorting free hash_a (it should be in place) and reassign hash_b to
     * hash_a */
    g_free(hash_a);
    hash_a = hash_b;
...
gboolean sensors_applet_settings_save_sensors(SensorsApplet *sensors_applet) {
  ...
  gdouble current_low_value, current_high_value, current_multiplier,
      current_offset;
  guint current_alarm_timeout, current_sensor_type, current_icon_type;
  ...
  /* now step through the GtkTreeStore sensors to find which sensors are
   * available / loaded */
  for (not_end_of_interfaces = gtk_tree_model_get_iter_first(
           GTK_TREE_MODEL(sensors_applet->sensors), &interfaces_iter);
       not_end_of_interfaces;
    ...
    for (not_end_of_sensors = gtk_tree_model_iter_children(
         ...
         not_end_of_sensors = gtk_tree_model_iter_next(
             GTK_TREE_MODEL(sensors_applet->sensors), &sensors_iter)) {
      gtk_tree_model_get(
          GTK_TREE_MODEL(sensors_applet->sensors), &sensors_iter, PATH_COLUMN,
          &current_path, ID_COLUMN, &current_id, LABEL_COLUMN, &current_label,
          INTERFACE_COLUMN, &current_interface, SENSOR_TYPE_COLUMN,
          &current_sensor_type, ENABLE_COLUMN, &current_enable,
          LOW_VALUE_COLUMN, &current_low_value, HIGH_VALUE_COLUMN,
          &current_high_value, ALARM_ENABLE_COLUMN, &current_alarm_enable,
          LOW_ALARM_COMMAND_COLUMN, &current_low_alarm_command,
          HIGH_ALARM_COMMAND_COLUMN, &current_high_alarm_command,
          ALARM_TIMEOUT_COLUMN, &current_alarm_timeout, MULTIPLIER_COLUMN,
          &current_multiplier, OFFSET_COLUMN, &current_offset, ICON_TYPE_COLUMN,
          &current_icon_type, GRAPH_COLOR_COLUMN, &current_graph_color, -1);

      /* GSettings unique id for one sensor data */
      gchar *gsuid = sensors_applet_settings_get_unique_id(
          current_interface, current_id, current_path);

      /* save sensor uid to gvariant array */
      g_variant_builder_add(&builder,
                            GSGVTS, /* must be related to the G_VARIANT_TYPE in
                                       init and gsettings schema */
                            gsuid);
      ...
      GSettings *settings;
      settings =
          g_settings_new_with_path("org.mate.sensors-applet.sensor", path);
      g_free(path);

      /* wait until g_settings_apply() is called to save all changes at once to
       * gsettings */
      ...
      g_settings_set_string(settings, LOW_ALARM_COMMAND,
                            current_low_alarm_command);
      g_settings_set_string(settings, HIGH_ALARM_COMMAND,
                            current_high_alarm_command);
   ...
   * the gvariant returned from g_variant_builder_end() is floating, so it is
   * freed by g_settings_set_value() */
  g_settings_set_value(sensors_applet->settings, "sensors-list",
