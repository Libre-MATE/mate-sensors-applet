      ...
      while ((file = g_dir_read_name(dir)) != NULL) {
        ...
        if ((handle = dlopen(plugin_file, RTLD_NOW)) != NULL) {
          ...
          if ((name_fn = dlsym(handle, "sensors_applet_plugin_name")) != NULL &&
              ...
              (get_value_fn = dlsym(
                   handle, "sensors_applet_plugin_get_sensor_value")) != NULL) {
            GList *sensors;
            ...
            if ((sensors = init_fn()) != NULL) {
              ...
              g_hash_table_insert(sensors_applet->plugins, g_strdup(name_fn()),
                                  get_value_fn);

              for (sensor = g_list_first(sensors); sensor != NULL;
                   sensor = g_list_next(sensor)) {
                SensorsAppletSensorInfo *sensor_info =
                    (SensorsAppletSensorInfo *)sensor->data;
                sensors_applet_add_sensor(
                    sensors_applet, sensor_info->path, sensor_info->id,
                    sensor_info->label, name_fn(), sensor_info->type,
                    sensor_info->enable, sensor_info->low_value,
                    sensor_info->high_value, FALSE, /* ALARM OFF */
                    "",                             /* no alarm commands */
                    "",                             /* no alarm commands */
                    0,                              /* alarm_timeout */
                    sensor_info->multiplier, sensor_info->offset,
                    sensor_info->icon, sensor_info->graph_color);

                // sensors_applet_add_sensor() doesn't free strings, so free
                // them here
              ...
              if (g_hash_table_lookup(sensors_applet->required_plugins,
                                      name_fn())) {
                g_debug(
                    "plugin is required - registering even though no sensors "
                    "detected");
                g_debug("registering plugin %s", name_fn());
                g_hash_table_insert(sensors_applet->plugins,
                                    g_strdup(name_fn()), get_value_fn);
              } else {
            ...
            g_debug("plugin file %s does not contain the required interface",
                    plugin_file);
            if (dlclose(handle) != 0) {
