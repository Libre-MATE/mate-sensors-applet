...
static void i8k_plugin_setup_manually(GList **sensors) {
  if (g_file_test(I8K_DEVICE_PATH, G_FILE_TEST_EXISTS)) {
    ...
    sensors_applet_plugin_add_sensor(sensors, I8K_DEVICE_PATH, "temp1",
                                     _("CPU"), TEMP_SENSOR, TRUE, CPU_ICON,
                                     DEFAULT_GRAPH_COLOR);

    sensors_applet_plugin_add_sensor(sensors, I8K_DEVICE_PATH, "fan1",
                                     _("FAN1"), FAN_SENSOR, FALSE, FAN_ICON,
                                     DEFAULT_GRAPH_COLOR);

    sensors_applet_plugin_add_sensor(sensors, I8K_DEVICE_PATH, "fan2",
                                     _("FAN2"), FAN_SENSOR, FALSE, FAN_ICON,
                                     DEFAULT_GRAPH_COLOR);
  }
}
...
gdouble i8k_plugin_get_sensor_value(const gchar *path, const gchar *id,
  ...
  if (NULL == (fp = fopen(path, "r"))) {
    g_set_error(error, SENSORS_APPLET_PLUGIN_ERROR, I8K_DEVICE_FILE_OPEN_ERROR,
  ...
  if (fscanf(fp, "%d %d %d %d %d", &cpu_temp, &fan1_status, &fan2_status,
             &fan1_rpm, &fan2_rpm) != 5) {
    g_set_error(error, SENSORS_APPLET_PLUGIN_ERROR, I8K_DEVICE_FILE_READ_ERROR,
  ...
  switch (type) {
    ...
    default:
      g_error(
          "Unknown sensor type passed as parameter to i8k sensor interface, "
          "cannot get value for this sensor");
      g_set_error(error, SENSORS_APPLET_PLUGIN_ERROR,
                  I8K_DEVICE_FILE_READ_ERROR,
                  "Error reading from sensor device file %s", path);
      return -1.0;
...
GList *sensors_applet_plugin_init(void) { return i8k_plugin_init(); }
