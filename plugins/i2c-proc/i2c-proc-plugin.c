...
enum { I2C_PROC_DEVICE_FILE_OPEN_ERROR, I2C_PROC_DEVICE_FILE_READ_ERROR };
...
static void i2c_proc_plugin_add_sensor(GList **sensors, const gchar *path) {
  ...
  if (g_ascii_strcasecmp(filename, "temp2") == 0) {
    ...
    switch (filename[0]) {
      ...
      default:
        ...
        g_debug(
            "error in i2c-proc sensor detection code - unhandled sensor "
            "filename %s",
            filename);
        g_free(filename);
        g_free(label);
  ...
  sensors_applet_plugin_add_sensor(sensors, path, filename, label, sensor_type,
                                   enable, icon_type, DEFAULT_GRAPH_COLOR);
...
static void i2c_proc_plugin_test_sensor(GList **sensors, const gchar *path) {
  ...
  if (g_str_has_prefix(filename, "curr") ||
      ...
      g_str_has_prefix(filename, "in") || g_str_has_prefix(filename, "temp") ||
...
static GList *i2c_proc_plugin_init(void) {
  ...
  sensors_applet_plugin_find_sensors(&sensors, I2C_PROC_BASE_DIR,
                                     i2c_proc_plugin_test_sensor);
  return sensors;
}
...
static gdouble i2c_proc_plugin_get_sensor_value(const gchar *path,
  ...
  if (NULL == (old_locale = setlocale(LC_NUMERIC, "C"))) {
    g_warning(
        "Could not change locale to C locale for reading i2c-proc device "
        "files.. will try regardless");
  }
    ...
    switch (type) {
      ...
      case VOLTAGE_SENSOR:
        ...
        switch (id[0]) {
          ...
          default:
            g_debug(
                "error in i2c-proc sensor read value function code - unhandled "
                "sensor id %s",
                id);
            g_set_error(error, SENSORS_APPLET_PLUGIN_ERROR,
                        I2C_PROC_DEVICE_FILE_READ_ERROR,
...
GList *sensors_applet_plugin_init(void) { return i2c_proc_plugin_init(); }
