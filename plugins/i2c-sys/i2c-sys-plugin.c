...
enum { I2C_SYS_DEVICE_FILE_OPEN_ERROR, I2C_SYS_DEVICE_FILE_READ_ERROR };
...
static void i2c_sys_plugin_add_sensor(GList **sensors, const gchar *path) {
  ...
  if (g_ascii_strcasecmp(filename, "temp2_input") == 0) {
    ...
    switch (filename[0]) {
      ...
      default:
        ...
        g_warning(
            "filename:\"%s\" begins with a charater that is not covered by "
            "this switch statement... not adding sensor",
            filename);
        g_free(filename);
        g_free(id);
        g_free(label);
  ...
  sensors_applet_plugin_add_sensor(sensors, path, id, label, sensor_type,
...
static GList *i2c_sys_plugin_init(void) {
  ...
  sensors_applet_plugin_find_sensors(&sensors, I2C_SYS_BASE_DIR,
...
GList *sensors_applet_plugin_init(void) { return i2c_sys_plugin_init(); }
