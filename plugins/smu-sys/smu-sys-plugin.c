...
enum { SMU_SYS_DEVICE_FILE_OPEN_ERROR, SMU_SYS_DEVICE_FILE_READ_ERROR };
...
GList *smu_sys_plugin_init(void) {
  ...
  sensors_applet_plugin_find_sensors(&sensors, SMU_SYS_BASE_DIR,
...
gdouble smu_sys_plugin_get_sensor_value(const gchar *path, const gchar *id,
  ...
  switch (type) {
    ...
    default:
      ...
      g_error(
          "Unknown sensor type passed as parameter to smu-sys sensor "
...
GList *sensors_applet_plugin_init(void) { return smu_sys_plugin_init(); }
