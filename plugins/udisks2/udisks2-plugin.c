...
static void update_device(DevInfo *info) {
  ...
  /* check for sensor_proxy, which should exist at this point, make one if
   * necessary and save it into DevInfo this is used to get the temp value the
   * direct way */
  if (NULL == info->sensor_proxy) {
    info->sensor_proxy = g_dbus_proxy_new_sync(
        connection, G_DBUS_PROXY_FLAGS_NONE, NULL, UDISKS2_BUS_NAME, info->path,
        UDISKS2_PROPERTIES_INTERFACE, NULL, &error);
   ...
   * it is possible to force a smart update with udisks2: SmartUpdate (IN  a{sv}
   * options); */

  /* directly asking the device's DBus object for the temp */
  tempgvar = g_dbus_proxy_call_sync(
      info->sensor_proxy, "Get",
      g_variant_new("(ss)", UDISKS2_DEVICE_INTERFACE2_NAME,
...
/* leaks memory! */
// syslog(LOG_ERR, "tempgvar value: %s",
// g_variant_print(g_variant_get_variant(g_variant_get_child_value(tempgvar,
// 0)), TRUE));
#endif

    /* tempgvar comes back as something along the lines of
     * array(gvariant(tempasdouble)) hence unpacking need to free up every param
     * / return value, so can't do it like: temp =
     * g_variant_get_double(g_variant_get_variant(g_variant_get_child_value(tempgvar,
     * 0))); */
 ...
 * it is possible with udisks2 to get all the above information in one
 * g_dbus_proxy_call_sync(), so that is how I did it maybe a better version
 * would be to use GDBusObjectManager Server + Client ?? */
static void udisks2_plugin_get_sensors(GList **sensors) {
  ...
  /* This connection will be used for everything, including the obtaining of
   * sensor data */
  ...
  proxy = g_dbus_proxy_new_sync(connection, G_DBUS_PROXY_FLAGS_NONE, NULL,
                                UDISKS2_BUS_NAME, UDISKS2_OBJECT_PATH,
                                UDISKS2_INTERFACE_NAME, NULL, &error);
  ...
  /* The object paths of the disks are enumerated and placed in an array of
   * object paths "GetManagedObjects" returns dict of (objectpath, (dict of
   * (string [ie. if. name], dict of(string [ie. property name], variant [ie.
   * prop. value])))) */
  ...
  managed_objects2 =
      g_dbus_proxy_call_sync(proxy, "GetManagedObjects", NULL, /* parameters */
                             G_DBUS_CALL_FLAGS_NONE,           /* flags */
                             -1,                               /* timeout */
                             NULL,                             /* cancellable */
                             &error);
...
#ifdef UD2PD
  // syslog(LOG_ERR, "managed_objects type: %s",
  // g_variant_print(managed_objects, TRUE));
  ...
  GVariantIter iter;
  gchar *key = NULL; /* object path (like
                        '/org/freedesktop/UDisks2/drives/Samsung_SSD_840_EVO_250GB_*insert
                        drive serial nr.*') */
  GVariant *value = NULL;

#ifdef UD2PD
  /* log collection size */
  syslog(LOG_ERR, "iter init count: %d",
         (int)g_variant_iter_init(&iter, managed_objects));
#else
   ...
   * changed to "{oa{sa{sv}}}" on error message 'the GVariant format string
   * '{sv}' has a type of '{sv}' but the given value has a type of
   * 'a{oa{sa{sv}}}'' a is array, o is object path NOO!! the right format string
   * is "{o@*}", which means get an object path into the 1st variable (key) and
   * get 'everything else' (as a GVariant) into the 2nd variable (value) needs
   * the & before the key and value params! */
...
// syslog(LOG_ERR, "value type: %s", g_variant_print(value, TRUE));
     ...
     * can't use GVariantDict, it only supports '{sv}' but the given value has a
     * type of '{sa{sv}}' using general lookup */
     ...
     */
    propdict = g_variant_lookup_value(value, UDISKS2_DEVICE_INTERFACE_NAME,
                                      G_VARIANT_TYPE_DICTIONARY);
    propdict2 = g_variant_lookup_value(value, UDISKS2_DEVICE_INTERFACE2_NAME,
                                       G_VARIANT_TYPE_DICTIONARY);
    ...
    if ((NULL != propdict) && (NULL != propdict2)) {
      ...
      id = (gchar *)g_variant_get_string(
          g_variant_lookup_value(propdict, "Id", G_VARIANT_TYPE_STRING), NULL);
      model = (gchar *)g_variant_get_string(
          g_variant_lookup_value(propdict, "Model", G_VARIANT_TYPE_STRING),
          NULL);

      smartenabled = g_variant_get_boolean(g_variant_lookup_value(
          propdict2, "SmartEnabled", G_VARIANT_TYPE_BOOLEAN));
      temp = g_variant_get_double(g_variant_lookup_value(
          propdict2, "SmartTemperature", G_VARIANT_TYPE_DOUBLE));
      ...
      if (smartenabled) {
        ...
        /* temp in K
         * this could be left at 0.0, 2 seconds later it will be refreshed
         * anyway */
        info->temp = (gdouble)temp - 273.15;
        g_hash_table_insert(devices, info->id, info);

        /* Write the sensor data */
        sensors_applet_plugin_add_sensor(sensors, id, "Disk Temperature", model,
...
GList *sensors_applet_plugin_init(void) { return udisks2_plugin_init(); }
