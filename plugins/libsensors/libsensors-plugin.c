...
static char *get_chip_name_string(const sensors_chip_name *chip) {
  ...
  switch (chip->bus) {
    ...
    case SENSORS_CHIP_NAME_BUS_DUMMY:
      name = g_strdup_printf("%s-%s-%04x", chip->prefix, chip->busname,
                             chip->addr);
      break;
    case SENSORS_CHIP_NAME_BUS_PCI:
      ...
      break;
    default:
      name = g_strdup_printf("%s-i2c-%d-%02x", chip->prefix, chip->bus,
                             chip->addr);
      break;
  }

#else
/* adapted from lm-sensors:prog/sensors/main.c:sprintf_chip_name in
 * lm-sensors-3.0 */
#define BUF_SIZE 200
...
static SensorType get_sensor_type(const char *name) {
  ...
  if (g_strrstr(name, "in")) {
    ...
    g_debug(
        "sensor %s not recognised as either voltage, fan or temp sensor - "
        "assuming is a current sensor",
        name);
    type = CURRENT_SENSOR;
  }
  return type;
...
static const sensors_feature_data *get_sensor_min_max(
    const sensors_chip_name *chip, int *n1, int *n2, int number,
    gdouble *low_value, gdouble *high_value) {
  ...
  while ((data = sensors_get_all_features(*chip, n1, n2)) != NULL &&
    ...
    if ((data->mode & SENSORS_MODE_R) &&
      ...
      if (g_str_has_suffix(data->name, "_min")) {
        *low_value = value;
        g_debug("overriding low value of sensor %s with value %f", data->name,
                value);
      } else if (g_str_has_suffix(data->name, "_max")) {
        *high_value = value;
        g_debug("overriding high value of sensor %s with value %f", data->name,
                value);
      }
    }
  }
...
#if SENSORS_API_VERSION < 0x400
static void check_sensor_with_data(GList **sensors, const char *const chip_name,
                                   const sensors_chip_name *chip, int *n1,
                                   int *n2, const sensors_feature_data *data) {
  char *label;
  double value;
  ...
  if ((label = get_sensor_interesting_label(*chip, data->number))) {
    ...
    if ((data->mode & SENSORS_MODE_R) &&
        ...
        (sensors_get_feature(*chip, data->number, &value) ==
         0)) { /* make sure we can actually get a value for it */
      ...
      /* the 'path' contains all the information we need to identify this sensor
       * later */
      url = g_strdup_printf("sensor://%s/%d", chip_name, data->number);

      /* get low and high values */
      sensors_applet_plugin_default_sensor_limits(type, &low_value,
                                                  &high_value);

      data = get_sensor_min_max(chip, n1, n2, data->number, &low_value,
                                &high_value);
      if (data != NULL) {
        /* try adding this one */
        /* at this point we have called sensors_get_all_features() and stopped
         * when we have a potential new sensor, so make sure we try this as well
         * - do a recursive call */
      ...
      sensors_applet_plugin_add_sensor_with_limits(
          sensors, url, label, label, type, visible, low_value, high_value,
          icon, DEFAULT_GRAPH_COLOR);
      g_free(url);
    }
    free(label);
  }
...
static GList *libsensors_plugin_get_sensors(void) {
  ...
  while ((chip_name = sensors_get_detected_chips(&i)) != NULL) {
    ...
    while ((data = sensors_get_all_features(*chip_name, &n1, &n2)) !=
           NULL) { /* error */
      /* fill in list for us */
      check_sensor_with_data(&sensors, chip_name_string, chip_name, &n1, &n2,
                             data);
    }
    ...
    while ((main_feature = sensors_get_features(chip_name, &nr1))) {
      switch (main_feature->type) {
        case SENSORS_FEATURE_IN:
          ...
          low_feature = sensors_get_subfeature(chip_name, main_feature,
                                               SENSORS_SUBFEATURE_IN_MIN);
          high_feature = sensors_get_subfeature(chip_name, main_feature,
      ...
      if (sensors_get_value(chip_name, input_feature->number, &value) < 0) {
        g_debug(
            "%s: %d: error: could not get value for input feature of sensor: "
            "%s\n",
      ...
      g_debug("for chip %s (type %s) got label %s and value %f",
              chip_name_string,
              (type == TEMP_SENSOR
                   ? "temp"
                   : (type == FAN_SENSOR
                          ? "fan"
                          : (type == VOLTAGE_SENSOR ? "voltage" : "error"))),
              label, value);

      path = g_strdup_printf("sensor://%s/%d", chip_name_string,
                             input_feature->number);
      g_hash_table_insert(hash_table, g_strdup(path), (void *)chip_name);

      sensors_applet_plugin_add_sensor_with_limits(&sensors, path, label, label,
                                                   type, visible, low, high,
                                                   icon, DEFAULT_GRAPH_COLOR);
    }
    g_free(chip_name_string);
  }
...
static gdouble libsensors_plugin_get_sensor_value(const gchar *path,
  ...
  if (g_match_info_matches(m)) {
    ...
    g_set_error(error, SENSORS_APPLET_PLUGIN_ERROR,
                LIBSENSORS_REGEX_URL_COMPILE_ERROR,
                "Error compiling URL regex: Not match");
  }
  g_match_info_free(m);
  return result;
}
...
static GList *libsensors_plugin_init(void) {
  ...
  uri_re = g_regex_new("^sensor://[a-z0-9_-]+/([0-9]+)$",
                       G_REGEX_CASELESS | G_REGEX_OPTIMIZE,
                       G_REGEX_MATCH_ANCHORED, &err);
  if (err) {
    g_debug(
        "Error compiling regexp: %s\nnot initing libsensors sensors interface",
        err->message);
    g_error_free(err);
    return NULL;
  }
...
GList *sensors_applet_plugin_init(void) { return libsensors_plugin_init(); }
