...
static const gchar *mbmon_plugin_query_mbmon_daemon(GError **error) {
  ...
  if (first_run ||
      current_query_time - previous_query_time > 2 * G_TIME_SPAN_SECOND) {
    int sockfd;
    ssize_t n;
    ...
    if ((sockfd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
      ...
      g_set_error(error, SENSORS_APPLET_PLUGIN_ERROR, MBMON_SOCKET_OPEN_ERROR,
                  "Error opening socket for mbmon");
      return NULL;
    }
    ...
    if (connect(sockfd, (struct sockaddr *)&address,
      ...
      g_set_error(error, SENSORS_APPLET_PLUGIN_ERROR,
                  MBMON_SOCKET_CONNECT_ERROR,
                  "Error connecting to mbmon daemon on port %i on %s",
                  htons(MBMON_PORT_NUMBER), MBMON_SERVER_IP_ADDRESS);
      return NULL;
    }

    pc = buffer;
    while ((n = read(sockfd, pc, MBMON_OUTPUT_BUFFER_LENGTH - output_length)) >
           0) {
      output_length += n;
      pc = &pc[n];
    }
...
static void mbmon_plugin_get_sensors(GList **sensors) {
  ...
  while (pv[0] != NULL) {
    ...
    sensors_applet_plugin_add_sensor_with_limits(
        sensors, name, name, label, type, visible, low_value, high_value, icon,
        DEFAULT_GRAPH_COLOR);
...
static gdouble mbmon_plugin_get_sensor_value(const gchar *path, const gchar *id,
...
GList *sensors_applet_plugin_init(void) { return mbmon_plugin_init(); }
