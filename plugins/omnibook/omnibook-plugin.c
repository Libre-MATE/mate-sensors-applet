...
enum { OMNIBOOK_DEVICE_FILE_OPEN_ERROR, OMNIBOOK_DEVICE_FILE_READ_ERROR };
...
static void omnibook_plugin_setup_manually(GList **sensors) {
  ...
  if (g_file_test(OMNIBOOK_DEVICE_FILE, G_FILE_TEST_EXISTS)) {
    sensors_applet_plugin_add_sensor(sensors, OMNIBOOK_DEVICE_FILE,
                                     _("temperature"), _("CPU"), TEMP_SENSOR,
...
static GList *omnibook_plugin_init(void) {
  GList *sensors = NULL;
  /* call function to recursively look for sensors starting at the defined base
   * directory */
...
GList *sensors_applet_plugin_init(void) { return omnibook_plugin_init(); }
