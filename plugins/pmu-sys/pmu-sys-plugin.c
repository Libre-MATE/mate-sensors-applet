...
enum { PMU_SYS_DEVICE_FILE_OPEN_ERROR, PMU_SYS_DEVICE_FILE_READ_ERROR };
...
static void pmu_sys_plugin_add_sensor(GList **sensors, const gchar *path) {
  ...
  if (g_ascii_strcasecmp(filename, SENSOR1 TEMPERATURE) == 0 ||
    ...
    enable = TRUE;
    icon_type = CPU_ICON;
  } else if (g_ascii_strcasecmp(filename, SENSOR2 TEMPERATURE) == 0 ||
    ...
    enable = TRUE;
    icon_type = CASE_ICON;
  } else if (g_ascii_strcasecmp(filename, SENSOR1 FAN_SPEED) == 0 ||
  ...
  if (enable) {
    sensors_applet_plugin_add_sensor(sensors, path, filename, label,
                                     sensor_type, enable, icon_type,
                                     DEFAULT_GRAPH_COLOR);
  }
...
GList *pmu_sys_plugin_init(void) {
  ...
  sensors_applet_plugin_find_sensors(&sensors, PMU_SYS_BASE_DIR,
                                     pmu_sys_plugin_test_sensor);
  sensors_applet_plugin_find_sensors(&sensors, PMU_WT_SYS_BASE_DIR,
                                     pmu_sys_plugin_test_sensor);
  return sensors;
}
...
gdouble pmu_sys_plugin_get_sensor_value(const gchar *path, const gchar *id,
  ...
  switch (type) {
    ...
    default:
      ...
      g_error(
          "Unknown sensor type passed as parameter to pmu-sys sensor "
...
GList *sensors_applet_plugin_init(void) { return pmu_sys_plugin_init(); }
