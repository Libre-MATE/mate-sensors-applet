...
static const gchar *hddtemp_plugin_query_hddtemp_daemon(GError **error) {
  ...
  seconds */
  if (first_run ||
      current_query_time - previous_query_time > G_TIME_SPAN_MINUTE) {
    int sockfd;
    ssize_t n;
    gchar *pc;
    ...
    if ((sockfd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
      ...
      g_set_error(error, SENSORS_APPLET_PLUGIN_ERROR, HDDTEMP_SOCKET_OPEN_ERROR,
                  "Error opening socket for hddtemp");
      return NULL;
    }
    ...
    if (connect(sockfd, (struct sockaddr *)&address,
      ...
      g_set_error(error, SENSORS_APPLET_PLUGIN_ERROR,
                  HDDTEMP_SOCKET_CONNECT_ERROR,
                  "Error connecting to hddtemp daemon on port %i on %s",
                  htons(HDDTEMP_PORT_NUMBER), HDDTEMP_SERVER_IP_ADDRESS);
      close(sockfd);
      return NULL;
    }
...
static void hddtemp_plugin_get_sensors(GList **sensors) {
  ...
  if (hddtemp_output[0] != '|') {
    g_debug(
        "Error in format of string returned from hddtemp daemon: char at [0] "
        "should be \"|\", instead whole output is: \"%s\"",
        hddtemp_output);
    return;
  }
  ...
  */
  ...
  while (pv[1] != NULL) {
    if (g_strcmp0(pv[2], "") != 0 && g_strcmp0(pv[3], "") != 0 &&
        g_strcmp0(pv[4], "") != 0 &&
...
GList *sensors_applet_plugin_init(void) { return hddtemp_plugin_init(); }
