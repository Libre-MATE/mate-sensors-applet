...
static gdouble netbsd_plugin_get_sensor_value(const gchar *path,
  ...
  while ((obj = prop_object_iterator_next(iter)) != NULL) {
    if (strcmp(path, prop_dictionary_keysym_cstring_nocopy(obj)) != 0) continue;

    array = prop_dictionary_get_keysym(dict, obj);
    siter = prop_array_iterator(array);
    while ((sobj = prop_object_iterator_next(siter)) != NULL) {
      if (prop_dictionary_get_cstring_nocopy(sobj, "description", &desc) ==
          false)
        continue;
      if (strcmp(id, desc) != 0) continue;
...
static void netbsd_plugin_get_sensors(GList **sensors) {
  ...
  while ((obj = prop_object_iterator_next(iter)) != NULL) {
    ...
    while ((sobj = prop_object_iterator_next(siter)) != NULL) {
      ...
      else if (strcmp(type, "Voltage AC") == 0 ||
               strcmp(type, "Voltage DC") == 0)
      ...
      sensors_applet_plugin_add_sensor(sensors, g_strdup(path), g_strdup(desc),
                                       g_strdup(desc), sensortype, TRUE,
                                       icontype, DEFAULT_GRAPH_COLOR);
    }
    prop_object_iterator_release(siter);
  }
  prop_object_iterator_release(iter);
...
/* API functions */
const gchar *sensors_applet_plugin_name(void) { return "netbsd"; }

GList *sensors_applet_plugin_init(void) { return netbsd_plugin_init(); }
