...
enum { SONYPI_DEVICE_FILE_OPEN_ERROR, SONYPI_DEVICE_FILE_READ_ERROR };
...
GList *sonypi_plugin_init(void) {
  ...
  if ((fd = open(SONYPI_DEV, O_RDONLY)) != -1) {
    if (close(fd) != -1) {
      sensors_applet_plugin_add_sensor(&sensors, SONYPI_DEV, SONYPI_TEMP,
                                       _("CPU TEMP"), TEMP_SENSOR, TRUE,
...
gdouble sonypi_plugin_get_sensor_value(const gchar *path, const gchar *id,
  ...
  if ((fd = open(path, O_RDONLY)) != -1) {
    ...
    g_set_error(error, SENSORS_APPLET_PLUGIN_ERROR,
                ...
                "Error opening from sensor device file %s", path);
  }
...
GList *sensors_applet_plugin_init(void) { return sonypi_plugin_init(); }
